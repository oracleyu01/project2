...
# 검색 입력
query = st.text_input("질문 입력", value="안성탕면 맛있게 끓이는 방법이 뭐지?", help="안성탕면에 관한 질문을 입력하세요")
...
with col3:
    show_raw_results = st.checkbox("원본 검색 결과 표시", value=False)
...
st.sidebar.info("""
1. 질문 입력: 안성탕면에 관한 질문을 입력하세요 (예: "안성탕면 맛있게 끓이는 방법이 뭐지?")
2. 검색 결과 수: 시스템이 참고할 관련 문서의 개수를 설정하세요.
3. 유사도 임계값: 검색 결과의 최소 유사도를 설정하세요. 높을수록 더 관련성 높은 결과만 사용됩니다.
4. 원본 검색 결과 표시: 체크하면 GPT 답변 아래에 원본 검색 결과도 함께 표시됩니다.
""")
